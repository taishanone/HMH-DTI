...
def argparser():
    ...
    parser.add_argument('--num_epochs', type=int, help='Number of epochs to train', default=2000)
    ...
    parser.add_argument('--target_aff_k', type=int, help='Affinity topk of target', default=150)
...
class DTADataset(InMemoryDataset):
    ...
    def process(self, drug_ids, target_ids, y):
        ...
        self.data = data_list #将每个样本变成一个对象
...
class GraphDataset_Drug_target(InMemoryDataset):
    def __init__(self, root='/tmp', transform=None, pre_transform=None, graphs_dict=None, dttype=None):
        super(GraphDataset_Drug_target, self).__init__(root, transform, pre_transform)
        self.dttype = dttype
    ...
    def process(self, graphs_dict):
        data_list = []
        for size, features, edge_index in graphs_dict:
...
def train(architecture, predictor, device, train_loader, drug_graphs_DataLoader, target_graphs_DataLoader, LR, epoch, TRAIN_BATCH_SIZE, Hgraph,drug_smiles_batchs,target_ESM2_batchs):
    ...
    drug_smiles_embedding_batchs = torch.stack(drug_smiles_batchs)
    drug_smiles_embedding_batchs=drug_smiles_embedding_batchs.float()
    drug_smiles_embedding_batchs=drug_smiles_embedding_batchs.to(device)

    target_ESM2_baths = torch.stack(target_ESM2_batchs)
    target_ESM2_baths=target_ESM2_baths.float()
    target_ESM2_baths=target_ESM2_baths.to(device)
    ...
    for batch_idx, data in enumerate(train_loader):
        optimizer.zero_grad()
        drug_embedding, target_embedding = architecture(Hgraph, drug_graph_batchs,drug_smiles_embedding_batchs, target_graph_batchs, target_ESM2_baths)
        output = predictor(data.to(device), drug_embedding, target_embedding,True)

        output = torch.sigmoid(output)
        loss=torch.nn.functional.binary_cross_entropy(output[:,1],data.y.view(-1).float().to(device))
        loss.backward()
        optimizer.step()
...
def predicting(architecture, predictor, device, loader, drug_graphs_DataLoader, target_graphs_DataLoader, Hgraph,drug_smiles_batchs,target_ESM2_batchs, drug_map=None, drug_map_weight=None, target_map=None, target_map_weight=None):
    architecture.eval()
    predictor.eval()
    ...
    with torch.no_grad():
        for data in loader:
            drug_embedding, target_embedding = architecture(
                Hgraph, drug_graph_batchs,drug_smiles_embedding_batchs, target_graph_batchs,target_ESM2_baths,
                drug_map=drug_map, drug_map_weight=drug_map_weight, target_map=target_map, target_map_weight=target_map_weight
            )
            output = predictor(data.to(device), drug_embedding, target_embedding,False)
            output = torch.sigmoid(output)
            total_preds = torch.cat((total_preds, output.cpu()), 0)

            total_labels = torch.cat((total_labels, data.y.view(-1).cpu()), 0)
    return total_labels.float().numpy(), total_preds.numpy()
